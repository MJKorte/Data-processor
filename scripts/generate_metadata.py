...
def write_data(df, df_annotation, variable_list, file, ispatient):
    ...
    df_annotated_split = df_annotated_split.drop(['Variables', "Sample/patient"], axis = 1)
...
def reformat_logical(df, logical_variables, column, c_values, r_values):
    if column in logical_variables:
        valuelist = df[column].tolist()
        c = 0
        for v_01 in valuelist:
            if np.isnan(v_01) == True or int(v_01) != int(c_values[0]) and int(v_01) != int(c_values[1]):
                valuelist[c] = ''
            elif int(v_01)==0:
                valuelist[c] = str(int(v_01)).replace(, 'No')
            elif int(v_01)==1:
                valuelist[c] = str(int(v_01)).replace('1', 'Yes')
            c += 1
        return df[column] = valuelist
#Read excel file
...
for column in df:
    ...
    df = reformat_logical(df, list_01, column, c_values = ['0', '1'], r_values = ['No', 'Yes'])
